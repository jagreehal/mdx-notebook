...
:::tip
This tutorial calls Anthropic Claude for real when `ANTHROPIC_API_KEY` is set. Otherwise it falls back to a mocked agent loop, so the page always renders. Look for the `mocked: true` flag in the result JSON to tell which version you're seeing.
:::
